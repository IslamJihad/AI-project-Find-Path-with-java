...
public class Controller implements Initializable {
	...
	private ObservableList<String> searchTypes = FXCollections.observableArrayList();
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		try {
			...
			searchTypes.add("A* (A)");
			searchTypes.add("A* (W)");
			searchTypes.add("Greedy");
			searchType.setValue("UCS");
	...
	/*
	 * fill the map with cities by putting each city on coordinates. each city with
	 * its name and a red circle that represents it
	 */
	...
	/*
	 * clicking run get the shortest path
	 */
	@FXML
	void run(ActionEvent event) {
		...
		if (source.getValue() != "none" && target.getValue() != "none") {
			...
			if (!getCity(source.getValue()).equals(getCity(target.getValue()))) {
				...
				JOptionPane.showMessageDialog(null, "You are already in the city", "heeeey!",
	...
	private void getShortestPath(City sourceCity, City targetCity) {
		...
		} else if (searchType.getValue().equals("A* (A)")) {
			AStarA(sourceCity, targetCity);
		} else if (searchType.getValue().equals("A* (W)")) {
			AStarW(sourceCity, targetCity);
		} else if (searchType.getValue().equals("Greedy")) {
			AStarA(sourceCity, targetCity);
		}

		// remove the previous search lines
		for (int i = 0; i < lines.size(); i++) {
		...
		// reset the total distance, time TextField
	...
	/*
	 * applying the dijkstra / UCS algorithm
	 */
	private void UCS(City source, City targetCity) {
		...
		while (!q.isEmpty()) {
			...
			for (Adjacent c : adj) {
				...
				// && are adjacent
				double newDis = c.getCarDistance() + temp.getDistance();
	...
	/*
	 * Build the hash table by applying the A* with areial heurestic algorithm
	 */

	private void AStarA(City source, City targetCity) {
		table.clear();
		for (City i : cities) {
			table.put(i.getName(), new Node(i, false, Double.POSITIVE_INFINITY, null));
		}
		...
		AStarCity city = new AStarCity(table.get(source.getName()), 0.0);
		city.getCity().setDistance(0.0);
		city.getCity().setKnown(true);
		city.setHeuristic(0.0);
		q.add(city);
		time = 0;
		while (!q.isEmpty()) {
			AStarCity temp = q.poll();
			temp.getCity().setKnown(true);
			if (temp.getCity().getCurrentCity() == targetCity) {
				time++;
				break;
			}
			ArrayList<Adjacent> adj = temp.getCity().getCurrentCity().getAdjacent();

			for (Adjacent c : adj) {
				AStarCity t = new AStarCity(table.get(c.getCity().getName()));
				if (t.getCity().isKnown()) {
					time++;
					continue;
				}
				time++;
				...
				// previous path distance
				// && are adjacent
				double newDis = c.getCarDistance() + temp.getCity().getDistance();
				if (newDis < t.getCity().getDistance()) {
					t.getCity().setSourceCity(temp.getCity().getCurrentCity());
					t.getCity().setDistance(newDis);
				}
				double xmin = targetCity.getCoordinateX() - t.getCity().getCurrentCity().getCoordinateX();
				double ymin = targetCity.getCoordinateY() - t.getCity().getCurrentCity().getCoordinateY();
				double heuristic = t.getCity().getDistance() + Math.sqrt(Math.pow(xmin, 2) + Math.pow(ymin, 2)) / 3;
	...
	private void getCities() throws FileNotFoundException {
		File file = new File("cities cordinates.csv");
	...
	private void getDistances() throws FileNotFoundException {
		File file = new File("distance of cities.csv");
		Scanner input = new Scanner(file);
	...
	private void Greedy(City source, City targetCity) {
		...
		while (!q.isEmpty()) {
			...
			for (Adjacent c : adj) {
				...
				double newDis = heuristic + temp.getCity().getDistance();
				if (newDis < t.getCity().getDistance()) {
					t.getCity().setSourceCity(temp.getCity().getCurrentCity());
					t.getCity().setDistance(newDis);
				}
	...
	private void AStarW(City source, City targetCity) {
		table.clear();
		for (City i : cities) {
			table.put(i.getName(), new Node(i, false, Double.POSITIVE_INFINITY, null));
		}
		...
		while (!q.isEmpty()) {
			...
			for (Adjacent c : adj) {
				...
				double xmin = targetCity.getCoordinateX() - t.getCity().getCurrentCity().getCoordinateX();
				double ymin = targetCity.getCoordinateY() - t.getCity().getCurrentCity().getCoordinateY();
				double WalkHur = ((Math.sqrt(Math.pow(xmin, 2) + Math.pow(ymin, 2)) / 3) + c.getCarDistance()) / 2;
				double heuristic = t.getCity().getDistance() + WalkHur;
				t.setHeuristic(heuristic);
